...
export function LayersPanel({ mapRef }: LayersPanelProps) {
    ...
    const [sublayerParentId, setSublayerParentId] = useState<NodeId | null>(
        null,
    );
        ...
        <div className='flex h-full min-h-0 flex-col'>
                                ...
                                {rows.map((row) => (
                                    <SortableRow
                                        ...
                                        onSelect={() => {
                                            ...
                                            if (row.node.kind === 'place')
                                                selectPlace(row.id);
                                            else selectPlace(null);
                                        }}
                                        onToggleVisible={() =>
                                            toggleLayerVisible(row.id)
                                        }
                                        onToggleCollapsed={() => {
            ...
            <PromptModal
                ...
                onConfirm={(name) => {
                    const parent =
                        selectedNodeIds[0] &&
                        document.nodes[selectedNodeIds[0]]?.kind === 'layer'
                            ? selectedNodeIds[0]
                            : null;
