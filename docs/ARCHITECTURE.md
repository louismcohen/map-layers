...
map-layers/
  ...
  AGENTS.md
  railway.toml              # Railway: build SPA, serve dist (not vite)
  .cursor/rules/
...
- **`apps/web`**: Mapbox UI, Zustand store wiring, search client, pin components adapted from yelp-combinator. Static Vite SPA — no app Node server in v1; `pnpm build` → `apps/web/dist`. Railway serves that folder with `serve` (not `vite`).
...
- Dialog: **profile** (walking / cycling / driving) + **metric** (time / distance) as horizontal **shadcn Tabs** with Phosphor icons + **amount** number input with a **shadcn Slider** beside it (minutes or miles; both stay in sync). Single contour; miles converted to meters for the API. Limits: 1–60 min, up to 60 mi (floored to nearest 5 under Mapbox’s ~62.1 mi / 100 km cap).
...
| API    | `generalize`           | Douglas–Peucker tolerance (m), scaled from estimated contour radius × profile fraction; clamp `[1, 200]` (15 min walk ≈ 5 m, drive capped at 200 m) |
| Client | `@turf/polygon-smooth` | Chaikin iterations `1–3`, interpolated from the same generalize scale (low → 1, drive cap → 3)                                         |
...
Static Vite SPA (no cookie session, no server loaders). `pnpm build` → `apps/web/dist`. Host on **Vercel**, **Cloudflare Pages**, **Railway**, or any static file host with the four `VITE_*` vars.

**Railway:** this is a pnpm workspace. Do **not** run `vite` / `pnpm dev` in production (that is the local HMR server; it will refuse `/proc/self/environ` and will not listen on `$PORT`). Config-as-code in [`railway.toml`](../railway.toml) (repo root) and [`apps/web/railway.toml`](../apps/web/railway.toml) (workspace package auto-import) sets `pnpm build` then `pnpm --filter @map-layers/web exec serve` on `0.0.0.0:$PORT` (direct `serve`, not `pnpm start`, so pnpm’s `$ script` banner is not written to stderr and tagged as an error). Set the four `VITE_*` vars on the **service** (available at build time; Vite inlines them — a value change needs a redeploy). Leave the service root as the **repo** (shared monorepo), not `apps/web` alone, so `@map-layers/domain` resolves.
