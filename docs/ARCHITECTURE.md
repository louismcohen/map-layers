...
**Railway:** this is a pnpm workspace. Do **not** run `vite` / `pnpm dev` in production (that is the local HMR server; it will refuse `/proc/self/environ` and will not listen on `$PORT`). Config-as-code in [`railway.toml`](../railway.toml) (repo root) and [`apps/web/railway.toml`](../apps/web/railway.toml) (workspace package auto-import) sets `pnpm build` then `pnpm start`, which serves `apps/web/dist` with [`serve`](https://www.npmjs.com/package/serve) on `0.0.0.0:$PORT`. Set the four `VITE_*` vars on the **service** (available at build time; Vite inlines them — a value change needs a redeploy). Leave the service root as the **repo** (shared monorepo), not `apps/web` alone, so `@map-layers/domain` resolves.
